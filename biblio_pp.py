    ...
    while True:                                                         
        match opcion_menu():                            #CREO EL MATCH DE LA FUNCION OPCION MENU PARA COMPARAR CON CADA CASE QUE LLAMARA A LA FUNCION CORRESPONDIENTE
            ...
            case "3":
                ...
                if bandera_ejercicio_2:                                                  #SI SE MODIFICO LA BANDERA DEL EJERCICIO 2  ACCEDO A GUARDAR DOCUMENTO
                    guardar_usuario = input("\nDesea guardar el ejercicio 2 en un archivo csv para ver más tarde?  Y/N: ")      #PREGUNTO AL USUARIO SI DESEA GUARDAR EL ARCHIVO 
                    patron = re.match(r'\b[yY]$|\b[nN]$', guardar_usuario)                                                  #VALIDO Y/N POR REGEX
                    while not patron:
                        guardar_usuario = input("\nERROR. Ingrese una respuesta válida: Y/N ")      
                        patron = re.match(r'\b[yY]$|\b[nN]$', guardar_usuario)
                    guardar_csv = exporta_csv('est_jugador.csv', jugador_datos)                                 #LLAMO A LA FUNCION EXPORTA_CSV Y LE PASO EL NOMBRE DEL ARCHIVO Y EL CONTENIDO
                    print(guardar_csv)                                                  #IMPRIMO EL RETORNO DE LA FUNCION
            ...
            case "7":
                maximo_rebotero = buscar_maximo(dicci, "rebotes_totales")       #LLAMO A LA FUNCION PARA AVERIGUAR EL MAXIMO REBOTERO, LE PASO DICCIONARIO Y ESTADISSTICA "REBOTES TOTALES"
                print(maximo_rebotero)                                          #IMPRIMO EL RETORNO DE LA FUNCION
            case "8":
                mayor_porcentaje_tiros_campo = buscar_maximo(dicci, "porcentaje_tiros_de_campo")    #LLAMO A LA FUNCION BUSCAR_MAXIMO PARA AVERIGUAR EL JUGADOR CON MAYOR PORCENTAJE TIROS DE CAMPO, LE PASO DICCIONARIO Y ESTADISTICA "PORCENTAJE_TIROS_DE_CAMPO"
                print(mayor_porcentaje_tiros_campo)                                                 # IMPRIMO EL RETORNO DE LA FUNCION
            case "9":
                asistencias_totales = buscar_maximo(dicci, "asistencias_totales")             #LLAMO A LA FUNCION BUSCAR_MAXIMO PARA AVERIGUAR EL MAYOR ASISTENTE, LE PASO DICCIONARIO Y ESTADISTICA "ASISTENCIAS_TOTALES"
                print(asistencias_totales)                                                    #IMPRIMO EL RETORNO DE LA FUNCION
            case "10":
                valor_ppp = input("Ingrese el ppp que desee: ")               #EL USUARIO INGRESA EL PPP QUE DESEE PARA COMPARAR QUE JUGADOR SUPERA ESE VALOR
                verif_valor = validar_numero_dos_digito(valor_ppp)            #VALIDO POR REGEX
                while verif_valor == -1:
                    valor_ppp = input("ERROR. Ingrese ppp valido: ")
                    verif_valor = validar_numero_dos_digito(valor_ppp)
                mayores = mayor_que_ingresado(dicci, float(valor_ppp) , "promedio_puntos_por_partido")  #LLAMO A LA FUNCION MAYOR_QUE_INGRESADO PARA AVERIGUAR LOS MAYORES AL DATO INGRESADO, LE PASO DICCIONARIO, EL VALOR INGRESADO Y ESTADISTICA promedio_puntos_por_partido
                print(mayores)                                          #IMPRIMO EL RETORNO DE LA FUNCION
            
            case "11":
                valor_rpp = input("Ingrese el rpp que desee: ")         #EL USUARIO INGRESA EL RPP QUE DESEE PARA COMPARAR QUE JUGADOR SUPERA ESE VALOR  
                verif_valor = validar_numero_dos_digito(valor_rpp)      #VALIDO POR REGEX
                while verif_valor == -1:
                    valor_rpp = input("ERROR. Ingrese rpp valido")          
                    verif_valor = validar_numero_dos_digito(valor_rpp)
                mayores = mayor_que_ingresado(dicci, float(valor_rpp) , "promedio_rebotes_por_partido")   #LLAMO A LA FUNCION MAYOR_QUE_INGRESADO PARA AVERIGUAR LOS MAYORES AL DATO INGRESADO, LE PASO DICCIONARIO, EL VALOR INGRESADO Y ESTADISTICA promedio_rebotes_por_partido 
                print(mayores)                                                                            #IMPRIMO EL RETORNO DE LA FUNCION
            case "12":
                valor_app = input("Ingrese el app que desee: ")         #EL USUARIO INGRESA EL APP QUE DESEE
                verif_valor = validar_numero_dos_digito(valor_app)
                while verif_valor == -1:
                    valor_app = input("ERROR. Ingrese app valido")
                    verif_valor = validar_numero_dos_digito(valor_app)
                mayores = mayor_que_ingresado(dicci, float(valor_app) , "promedio_asistencias_por_partido") #LLAMO A LA FUNCION MAYOR_QUE_INGRESADO PARA AVERIGUAR LOS MAYORES AL DATO INGRESADO, LE PASO DICCIONARIO, EL VALOR INGRESADO Y ESTADISTICA promedio_asistencias_por_partido 
                print(mayores)                                                                              #IMPRIMO EL RETORNO DE LA FUNCION
            case "13":
                mayor_robador = buscar_maximo(dicci, "robos_totales")           #LLAMO A LA FUNCION PARA BUSCAR EL MAXIMO ROBADOR, LE PASO DICCIONARIO Y ESTADISTICAS "ROBOS_TOTALES"
                print(mayor_robador)                                            #IMPRIMO EL RETORNO DE LA FUNCION
            case "14":
                mayor_bloqueador = buscar_maximo(dicci, "bloqueos_totales")     #LLAMO A LA FUNCION PARA BUSCAR EL MAXIMO BLOQUEADOR, LE PASO DICCIONARIO Y ESTADISTICAS "BLOQUEOS_TOTALES"
                print(mayor_bloqueador)                                         #IMPRIMO EL RETORNO DE LA FUNCION
            case "15":
                valor_tlpp = input("Ingrese el tlpp que desee: ")             #EL USUARIO INGRESA EL VALOR DE PORCENTAJE TLPP QUE DESEA COMPARAR
                verif_valor = validar_numero_dos_digito(valor_tlpp)           #VERIFICO POR REGEX
                while verif_valor == -1:                                        
                    valor_tlpp = input("ERROR. Ingrese tlpp valido: ")
                    verif_valor = validar_numero_dos_digito(valor_tlpp)
                mayores = mayor_que_ingresado(dicci, float(valor_tlpp) , "porcentaje_tiros_libres") #LLAMO A LA FUNCION MAYOR_QUE_INGRESADO PARA AVERIGUAR LOS MAYORES AL DATO INGRESADO, LE PASO DICCIONARIO, EL VALOR INGRESADO Y ESTADISTICA porcentaje_tiros_libres 
                print(mayores)                                                                      #IMPRIMO EL RETORNO DE LA FUNCION
            case "16":
                ppp_equipo = promedio_pp_excluido(dicci)        #LLAMO A LA FUNICON PROMEDIO_PP_EXCLUIDO PARA AVERIGUAR EL PPP DEL EQUIPO Y EXCLUIR AL MINIMO, LE PASO DICCIONARIO
                print(ppp_equipo)                               #IMPRIMO EL RETORNO DE LA FUNCION
            case "17":
                mayor_logros = mayor_cant_logros(dicci)         #LLAMO A MAYOR_CANT_LOGROS
                print(mayor_logros)                             #IMPRIMO RETORNO EN PANTALLA
                
            case "18":
                valor_ttpp = input("Ingrese el porcentaje de tiros triples que desee: ")   #EL USUARIO INGRESA EL PORCENTAJE DE TIROS TRIPLES QUE DESEA COMPARAR
                verif_valor = validar_numero_dos_digito(valor_ttpp)                        #VERIFICO POR REGEX
                while verif_valor == -1:
                    valor_ttpp = input("ERROR. Ingrese ttpp valido: ")
                    verif_valor = validar_numero_dos_digito(valor_ttpp)
                mayores = mayor_que_ingresado(dicci, float(valor_ttpp) , "porcentaje_tiros_triples")   #LLAMO A LA FUNCION MAYOR_QUE_INGRESADO PARA AVERIGUAR LOS MAYORES AL DATO INGRESADO, LE PASO DICCIONARIO, EL VALOR INGRESADO Y ESTADISTICA porcentaje_tiros_triples 
                print(mayores)                                                                         #IMPRIMO EL RETORNO DE LA FUNCION
                
            case "19":
                mayor_temporadas_jugadas = buscar_maximo(dicci, "temporadas")               #LLAMO A LA FUNCION BUSCAR MAXIMO PARA AVERIGUAR QUE JUGADOR TIENE MAS TEMPORADAS EN SU ESTADISTICA, LE PASO DICCIONARIO Y ESTADISTICAS "TEMPORADAS"
                print(mayor_temporadas_jugadas)                                             #IMPRIMO EL RETORNO DE LA FUNCION
            
            case "20":
                valor_ppp = input("Ingrese el porcentaje de tiros que campo que desee: ")        #EL USUARIO INGRESA EL PORCENTAJE DE TIROS TRIPLES QUE DESEA COMPARAR
                verif_valor = validar_numero_dos_digito(valor_ppp)                               #VERIFICO POR REGEX
                while verif_valor == -1:                                                   
                    valor_ppp = input("ERROR. Ingrese ppp valido: ")
                    verif_valor = validar_numero_dos_digito(valor_ppp)
...
def jugador_por_nombre_logros(dato:dict):                  # TOMA EL VALOR DEL DICCIONARIO QUE SE PASE Y DEVUELVE
    ...
    return retorno                                          #RETORNO EL DICCIONARIO RETORNO
...
def quicksort_basico(lista):                        #TRAIGO LA LISTA QUE DESEO ORDENAR
    ...
    lista_ordenada.extend(menores_ordenados)        # UTILIZO .EXTEND PARA AGREGAR LOS VALORES MENORES YA QUE DE ESTA FORMA ME IRA AGREGANDO ORDENADAMENTE LOS VALORES, SE LO CONTRARIO ENTRARIA EN UN BUCLE INFINITO PORQUE SE CARGARIAN COMO VIENE Y NUNCA TERMINARIA DE ORDENARSE
